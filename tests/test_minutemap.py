"""Tests for YearMinuteMap object"""
...
class TestYearMinuteMap(unittest.TestCase):
    def test_valid(self):
        cases: list[tuple[dict[str, Any], datetime, int | None, str]] = [
            ...
            (
                ...
                1,
                "mon alias (2024-01-01 is Monday)",
            ),
            (
                {"sun": 7},
                datetime(2024, 1, 7, 0, 0),
                7,
                "sun alias (2024-01-07 is Sunday)",
            ),
            # Ranges
            (
                {"moy1-3": 55},
                ...
                55,
                "moy range matches February",
            ),
            (
                {"moy1-3": 55},
                datetime(2024, 4, 1, 0, 0),
                None,
                "moy range does not match April",
            ),
            ...
            (
                {"h9-17": 5},
                datetime(2024, 3, 1, 18, 0),
                None,
                "h9-17 does not match 18:00",
            ),
            ...
            (
                ...
                1,
                "shallower path fallback",
            ),
            # Nested dict input
            (
                {"h19": {"*": 18, "m30-59": 22}},
                datetime(2024, 6, 1, 19, 0),
                18,
                "nested: h19.*",
            ),
            (
                {"h19": {"*": 18, "m30-59": 22}},
                datetime(2024, 6, 1, 19, 45),
                22,
                "nested: h19.m30-59",
            ),
            ...
            (
                ...
                datetime(2024, 1, 7, 9, 30),
                5,
                "specificity: full path wins over all shallower",
            ),
            (
                {"q1": 1, "jan": 2},
                datetime(2024, 1, 15, 0, 0),
                2,
                "specificity: MOY beats QTR at depth 1",
            ),
            (
                {"q1.dow1": 1, "q1.dom1": 2},
                datetime(2024, 1, 1, 0, 0),
                2,
                "specificity: DOM beats DOW at depth 2 (2024-01-01 is Monday)",
            ),
            (
                {"woy1": 1, "doy1": 2},
                datetime(2024, 1, 1, 0, 0),
                2,
                "specificity: DOY beats WOY at depth 1",
            ),
            (
                {"h9": {"*": 1, "m0": 2}},
                datetime(2024, 6, 1, 9, 1),
                1,
    ...
    def test_valid_float_value(self):
        cases = [
            ({"*": 7.7}, datetime(2024, 7, 4, 14, 0), 7.7, "bare wildcard"),
        ]
        for spec, dt, expected, description in cases:
            with self.subTest(description):
    ...
    def test_invalid_float_value(self):
        cases = [
            ({"*": 7}, "expected float", "not a float"),
        ]
        for spec, fragment, description in cases:
            with self.subTest(description):
                with self.assertRaises(ValueError) as ctx:
                    YearMinuteMap(spec, float)
                (self.assertIn(fragment, str(ctx.exception).lower(), description),)  # type: ignore
